...
public class IconManager : MonoBehaviour
{
    ...
    public void CheckMyIcon()
    {
        ...
        plusText.text = LocalizationManager.instance.GetString("NowPrice") + " +" + ((playerDataBase.GetIconHoldNumber() + 1) * reward).ToString() + "%  (+" + reward + "%)";
    ...
    public void CheckIconUnLock() //아이콘 언락 체크
    {
        ...
        number++;
        if (playerDataBase.ConsumeGold > 9 || playerDataBase.CheckIcon(IconType.Icon_1 + number)) //1000만 골드
        {
            GetIcon(number);
        }
